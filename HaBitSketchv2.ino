...
void loop()
{
  ...
  if (BLEbuffer.indexOf("Red") >= 0) {
    Serial.println(F("Status Request Received"));
    ble.print("AT+BLEUARTTX=");
    for (int i = 0; i < NUMPIXELS; ++i) {
      CircuitPlayground.setPixelColor(i, 255, 0, 0);
      CircuitPlayground.setBrightness(50);
      CircuitPlayground.playTone(50,50);
      CircuitPlayground.playTone(60,50);

    }
  }

    if (BLEbuffer.indexOf("Green") >= 0) {
    Serial.println(F("Status Request Received"));
    ble.print("AT+BLEUARTTX=");
    for (int i = 0; i < NUMPIXELS; ++i) {
